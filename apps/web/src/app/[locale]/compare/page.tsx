...
export default function ComparePage() {
    ...
    const [reports, setReports] = useState<Map<number, TaxSummary | null>>(new Map());
    ...
    const thStyle = {
        padding: '10px 16px', textAlign: 'left' as const, fontWeight: 600,
        borderBottom: '1px solid var(--border)', fontSize: '13px',
    };

    const tdStyle = {
        padding: '10px 16px', borderBottom: '1px solid var(--border)',
        fontSize: '14px', fontFamily: 'var(--font-mono, monospace)',
    };
    ...
    return (
        <div className="animate-in">
                        ...
                        {ALL_YEARS.map(y => (
                            <button
                                ...
                                style={{
                                    ...
                                    background: selectedYears.has(y) ? 'var(--accent)' : 'var(--bg-secondary)',
            ...
            {errors.length > 0 && (
                <div className="card" style={{ padding: '12px 16px', marginBottom: '16px', background: 'rgba(234,179,8,0.1)', border: '1px solid rgba(234,179,8,0.3)', borderRadius: 'var(--radius-sm)', fontSize: '13px', color: 'var(--text-muted)' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            {sortedYears.map(year => {
                                ...
                                return (
                                    <tr key={year}>
                                        <td style={{ ...tdStyle, fontWeight: 600, fontFamily: 'inherit' }}>{year}</td>
                                        <td style={{ ...tdStyle, color: 'var(--green-light, #22c55e)' }}>{formatUsd(r.shortTermGains)}</td>
                                        <td style={{ ...tdStyle, color: 'var(--red-light, #ef4444)' }}>{formatUsd(r.shortTermLosses)}</td>
                                        <td style={{ ...tdStyle, color: 'var(--green-light, #22c55e)' }}>{formatUsd(r.longTermGains)}</td>
                                        <td style={{ ...tdStyle, color: 'var(--red-light, #ef4444)' }}>{formatUsd(r.longTermLosses)}</td>
                                        <td style={{ ...tdStyle, fontWeight: 700, color: colorClass(net) }}>{formatUsd(net)}</td>
