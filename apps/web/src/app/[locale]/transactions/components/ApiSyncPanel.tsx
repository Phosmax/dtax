    ...
    <div className="card" style={{ marginBottom: "24px" }}>
      ...
      >
        ...
        <div>
          ...
          <form
            onSubmit={handleConnectApi}
            style={{ display: "flex", flexDirection: "column", gap: "12px" }}
          >
            <div>
              <label style={labelStyle}>{t("apiSync.exchange")}</label>
              <select
                value={apiForm.exchangeId}
                onChange={(e) =>
                  setApiForm({ ...apiForm, exchangeId: e.target.value })
                }
                style={inputStyle}
              >
            ...
            {(apiForm.exchangeId === "okx" ||
              apiForm.exchangeId === "kucoin") && (
              <div>
                <label style={labelStyle}>{t("apiSync.apiPassword")}</label>
                <input
                  type="password"
                  required
                  value={apiForm.apiPassword}
                  onChange={(e) =>
                    setApiForm({ ...apiForm, apiPassword: e.target.value })
                  }
                  style={inputStyle}
                />
              </div>
            ...
            {apiError && (
              <div style={{ color: "var(--red-light)", fontSize: "13px" }}>
                {apiError}
              </div>
            )}
            <button
