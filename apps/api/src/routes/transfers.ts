...
export async function transferRoutes(app: FastifyInstance) {
    ...
    app.get('/transfers/matches', async (_request, _reply) => {
        ...
        const records: TransferRecord[] = transfers.map(tx => ({
            ...
            asset: (tx.type === 'TRANSFER_OUT' ? tx.sentAsset : tx.receivedAsset) || '',
            amount: Number(tx.type === 'TRANSFER_OUT' ? tx.sentAmount : tx.receivedAmount) || 0,
    ...
    app.post('/transfers/confirm', async (request, reply) => {
        ...
        return { data: { status: 'confirmed', outTxId: body.outTxId, inTxId: body.inTxId } };
    });
