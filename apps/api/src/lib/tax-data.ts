...
const ACQUISITION_TYPES = [
    'BUY', 'TRADE', 'AIRDROP', 'STAKING_REWARD', 'MINING_REWARD', 'INTEREST',
    'FORK', 'GIFT_RECEIVED', 'DEX_SWAP', 'LP_WITHDRAWAL', 'LP_REWARD',
    'NFT_MINT', 'MARGIN_TRADE', 'LIQUIDATION',
];

/** Transaction types that trigger dispositions (taxable events) */
const DISPOSITION_TYPES = [
    'SELL', 'TRADE', 'GIFT_SENT', 'LOST', 'STOLEN', 'DEX_SWAP',
    'LP_DEPOSIT', 'NFT_PURCHASE', 'NFT_SALE', 'MARGIN_TRADE', 'LIQUIDATION',
];

/** Transaction types that count as ordinary income */
const INCOME_TYPES = ['STAKING_REWARD', 'MINING_REWARD', 'AIRDROP', 'INTEREST', 'LP_REWARD'];
 ...
 */
export async function fetchTaxData({ userId, taxYear }: TaxDataParams): Promise<TaxData> {
 ...
 */
export async function calculateIncome({ userId, taxYear }: TaxDataParams): Promise<IncomeBreakdown> {
    const yearStart = new Date(`${taxYear}-01-01T00:00:00Z`);
    const yearEnd = new Date(`${taxYear + 1}-01-01T00:00:00Z`);
    ...
    let staking = 0, mining = 0, airdrops = 0, interest = 0;
    ...
    return { staking, mining, airdrops, interest, total: staking + mining + airdrops + interest };
}

/**
 * Fetch internal transfer IDs for reconciliation misclassification detection.
 */
export async function fetchInternalTransferIds(userId: string, yearStart: Date, yearEnd: Date): Promise<Set<string>> {
    ...
    return new Set(transfers.map(t => t.externalId).filter((id): id is string => id !== null));
}
